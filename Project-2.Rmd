...
```{r results = "hide"}
data_t <- data[grep('^T', data$survey_company), ]

table(data_t$survey_company)
...
```{r results = "hide"}
data_a <- data[grep('^A', data$survey_company), ]

table(data_a$survey_company)

af_mapping = c("ABERCROMBIE" = "ABERCROMBIE & FITCH")
```

### JP Morgan

```{r results = "hide"}
data_j <- data[grep('^J', data$survey_company), ]

table(data_j$survey_company)

jpmorgan_mapping = c("J.P MORGAN" = "JP MORGAN CHASE",
                     "J.P. MORGAN" = "JP MORGAN CHASE",
                     "J.P. MORGAN U.S. PRIVATE BANK" = "JP MORGAN CHASE",
                     "JP MORGAN" = "JP MORGAN CHASE",
                     "JPMORGAN CHASE & CO." = "JP MORGAN CHASE", 
                     "JP MORGAN AND CHASE" = "JP MORGAN CHASE", 
...
```{r results = "hide"}
data_r <- data[grep('^R', data$survey_company), ]

table(data_r$survey_company)
```

### Coyote Logistics

```{r results = "hide"}
data_c <- data[grep('^C', data$survey_company), ]

table(data_c$survey_company)

coyote_mapping = c("COYOTELOGISITCS" = "COYOTE LOGISTICS",
...
data$survey_company <- str_replace_all(data$survey_company, textron_mapping)

data$survey_company <- str_replace_all(data$survey_company, af_mapping)

data$survey_company <- str_replace_all(data$survey_company, jpmorgan_mapping)

data$survey_company <- str_replace_all(data$survey_company, coyote_mapping)
```
...
We then imputed the average salary for each major per year for all responses with a missing salary response.
...
We only kept observations for students who said they have accepted a full-time or part-time job, ignoring observations for students continuing education, joining the military, etc..
